...
class HomeRotaViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
...
//            ]
        let rotas = dbHelper.fetchAllServicos()
        for rota in rotas {
            let nomeL = rota.1
            let precoL = rota.2
        ...
        addButton.addTarget(self, action: #selector(addRota), for: .touchUpInside)
    ...
    @objc func addRota(){
        let telas = AddRotaViewController()
        telas.modalPresentationStyle = .fullScreen
        self.navigationController?.pushViewController(telas, animated: true)
    }
